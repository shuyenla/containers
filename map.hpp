...
namespace ft {
    ...
    class map
	{
		...
		public:
	    	...
	    	map():_rbt(), _vc(Compare()), _a(Allocator()) {}
	    	explicit map(const Compare& comp, const Allocator& = Allocator()):_rbt(),_vc(comp), _a(Allocator()) {}
	    	template<class InputIt>
	    		map(InputIt first, InputIt last,
	    	    const Compare& comp = Compare(), const Allocator& = Allocator()):_rbt(), _vc(comp), _a(Allocator()) { insert(first, last); }
	    	map(const map& x):_rbt(), _vc(x._vc), _a(x._a) { insert(x.begin(), x.end()); }
	    	~map() {}
